...
EOF
  ...
  # 写入 /etc/environment 文件(系统级环境变量,对所有进程生效)
  local system_env="/etc/environment"
  echo "Writing system-wide environment: ${system_env}"
...
EOF
  ...
  # 更新 PATH 到 /etc/environment
  local path_line="PATH=\"/opt/java/current/bin:/opt/node/current/bin:/opt/gradle/current/bin:/opt/android-sdk/cmdline-tools/latest/bin:/opt/android-sdk/platform-tools:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin\""
  if grep -q "^PATH=" "$system_env" 2>/dev/null; then
    $SUDO sed -i "s|^PATH=.*|${path_line}|" "$system_env"
  else
    echo "$path_line" | $SUDO tee -a "$system_env" >/dev/null
  fi
  $SUDO chmod 644 "$system_env"
  echo "System-wide environment configured: $system_env"
  
  # 写入 ~/.bashrc (Docker 容器交互式 bash 主要加载此文件)
  local bashrc="$HOME/.bashrc"
  local bashrc_backup="${bashrc}.bak.$(date +%s)"
  ...
  # 检查是否已存在 Cordova 环境配置,避免重复添加
  if grep -q "# Cordova Build Environment" "$bashrc" 2>/dev/null; then
    echo "Cordova environment already configured in $bashrc, skipping..."
  else
    # 追加环境变量到 .bashrc
...
export PATH="/opt/java/current/bin:/opt/node/current/bin:/opt/gradle/current/bin:/opt/android-sdk/cmdline-tools/latest/bin:/opt/android-sdk/platform-tools:${PATH}"
BASHRC_EOF
    echo "Cordova environment added to: $bashrc"
  fi
  
  # 写入 /etc/bash.bashrc (系统级 bash 配置,所有用户生效)
  local system_bashrc="/etc/bash.bashrc"
  if [ -f "$system_bashrc" ]; then
    if grep -q "# Cordova Build Environment" "$system_bashrc" 2>/dev/null; then
      echo "Cordova environment already configured in $system_bashrc, skipping..."
    else
      cat >> "$system_bashrc" <<'SYSTEM_BASHRC_EOF'
...
SYSTEM_BASHRC_EOF
      echo "Cordova environment added to: $system_bashrc"
    fi
  fi
  
  # 写入 /etc/profile (确保登录 shell 也加载)
  local profile_append="/etc/profile.d/cordova-profile.sh"
  $SUDO tee "$profile_append" >/dev/null <<'PROFILE_EOF'
# Cordova Environment - Loaded by /etc/profile
if [ -f /etc/environment ]; then
  export $(grep -v '^#' /etc/environment | xargs)
fi
PROFILE_EOF
  $SUDO chmod 644 "$profile_append"
  ...
  echo "=========================================="
  echo "System-wide config: /etc/environment"
  echo "System bash config: /etc/bash.bashrc"
  echo "User bash config: ~/.bashrc"
  echo "Login shell config: /etc/profile.d/cordova-env.sh"
  echo ""
  echo "Note: Environment variables are automatically"
  echo "      reloaded by the Python service."
  echo "      New terminal sessions will load automatically."
  echo "=========================================="
}
